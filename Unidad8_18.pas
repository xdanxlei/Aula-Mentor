(*
	Fichero: Unidad8_18.pas
	Fecha: 17/05/2019
	Autor: Daniel García Zambrano
	Mail: metroid.xdan@gmail.com
	Descripción: recoge los datos de un número variable de cuadros searados en dos colecciones y los muestra por pantalla. 
		Indica la suma de sus precios y los precios máximo y mínimo. Permite buscar cuadros por autor.
	*)
...
Begin
	...
	writeln;
	repeat
		write('Numero de cuadros: ');
		readln(numcuadros1);
		if numcuadros1 > tamcoleccion then
		begin
			writeln;
			writeln('El numero de cuadros no puede exceder ', tamcoleccion, '.');
		end;
		writeln;
	until numcuadros1 <= tamcoleccion;
	lee_coleccion(coleccion1, numcuadros1);
	writeln;
	
	writeln('>Datos segunda coleccion:');
	writeln;
	repeat
		write('Numero de cuadros: ');
		readln(numcuadros2);
		if numcuadros2 > tamcoleccion then
		begin
			writeln;
			writeln('El numero de cuadros no puede exceder ', tamcoleccion, '.');
		end;
		writeln;
	until numcuadros2 <= tamcoleccion;
	lee_coleccion(coleccion2, numcuadros2);
	writeln;
	 
	(*-- Primera colección. --*)
	calcula_estadisticas(coleccion1, numcuadros1, suma, no_en_venta, preciomin, preciomax);
	
	writeln('>Datos de los cuadros:');
	writeln;
	
	writeln('Primera coleccion:');
	writeln;
	imprime_coleccion(coleccion1, numcuadros1);
	writeln;
	...
	(*-- Segunda colección. --*) 
	calcula_estadisticas(coleccion2, numcuadros2, suma, no_en_venta, preciomin, preciomax);
	
	writeln('Segunda coleccion:');
	writeln;
	imprime_coleccion(coleccion2, numcuadros2);
	writeln;
	...
	repeat
		...
		begin
			writeln('Primera coleccion:');
			writeln;
			busca_autor(coleccion1, autor_buscado, numcuadros1);
			writeln('Segunda coleccion:');
			writeln;
			busca_autor(coleccion2, autor_buscado, numcuadros2);
		end;
